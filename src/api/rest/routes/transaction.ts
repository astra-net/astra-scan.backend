...
transactionRouter.get(
  ...
  catchAsync(getTransactionBlockNumber)
);
...
export async function getTransactionBlockNumber(
  ...
  const txs = await controllers.getTransactionByField(
    s,
    "block_number",
    +blockNumber
  );
...
transactionRouter.get(
  ...
  catchAsync(getTransactionByBlockHash)
);

export async function getTransactionByBlockHash(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export async function getTransactions(
  ...
  const { offset, limit, orderBy, orderDirection, type, property, value } =
    req.query;
