...
const fixMissingBlocks = async (shardID: ShardID) => {
  ...
  const missingBlocks = await store.utils.getMissingBlocks(
    0,
    latestIndexedBlock!
  );

  l.info(
    `Found ${missingBlocks.length} missing blocks in [0,${latestIndexedBlock}] range`
  );
...
export const indexer = async () => {
  l.info(
    `Fix Missing Blocks Task starting... Shards[${config.indexer.shards.join(
      ", "
    )}]`
  );
