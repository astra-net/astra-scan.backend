...
const parseLogs = (
  ...
  const filteredLogs = logs.filter(({ topics }) =>
    topics.includes(eventSignature)
  );
...
const startMigration = async () => {
  ...
  l.info(
    `Completed loading contracts data: erc20 ${
      Object.keys(erc20Map).length
    }, erc721 ${Object.keys(erc721Map).length}, erc1155 ${
      Object.keys(erc1155Map).length
    }`
  );
