...
try {
  VERSIONFileData = `${fs.readFileSync(
...
export const config = {
  ...
  api: {
    shards: getCommaSeparatedList(process.env.API_SHARDS).map(
      (s) => +s
    ) as ShardID[],
  ...
  indexer: {
    ...
    shards: getCommaSeparatedList(process.env.INDEXER_SHARDS).map(
      (s) => +s
    ) as ShardID[],
    isEnabled: toBool(process.env.INDEXER_IS_ENABLED || "0"),
    isSyncingBlocksEnabled: toBool(
      process.env.INDEXER_BLOCKS_IS_ENABLED || "0"
    ),
    isSyncingLogsEnabled: toBool(process.env.INDEXER_LOGS_IS_ENABLED || "0"),
    isSyncingContractsEnabled: toBool(
      process.env.INDEXER_CONTRACTS_IS_ENABLED || "0"
    ),
    trackContractTypes: getCommaSeparatedList(
      process.env.INDEXER_CONTRACTS_TYPES
    ),
    initialBlockSyncingHeight: +(
      process.env.INDEXER_INITIAL_BLOCK_SYNCING_HEIGHT || 0
    ),
    // set to the height where smart contracts were introduced on the chain
    initialLogsSyncingHeight: +(
      process.env.INDEXER_LOG_INITIAL_BLOCK_SYNCING_HEIGHT || 3500000
    ),
...
export const init = async () => {
  ...
  if (toBool(process.env.AWS_CONFIG_IS_ENABLE)) {
    ...
    config.store.postgres[0].user = parseValue(
      decrypted,
      "SHARD0_POSTGRES_USER"
    );
    config.store.postgres[0].password = parseValue(
      decrypted,
      "SHARD0_POSTGRES_PASSWORD"
    );
    config.store.postgres[1].user = parseValue(
      decrypted,
      "SHARD1_POSTGRES_USER"
    );
    config.store.postgres[1].password = parseValue(
      decrypted,
      "SHARD1_POSTGRES_PASSWORD"
    );
    config.store.postgres[2].user = parseValue(
      decrypted,
      "SHARD2_POSTGRES_USER"
    );
    config.store.postgres[2].password = parseValue(
      decrypted,
      "SHARD2_POSTGRES_PASSWORD"
    );
    config.store.postgres[3].user = parseValue(
      decrypted,
      "SHARD3_POSTGRES_USER"
    );
    config.store.postgres[3].password = parseValue(
      decrypted,
