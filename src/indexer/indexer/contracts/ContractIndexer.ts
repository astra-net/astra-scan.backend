...
export class ContractIndexer {
  ...
  private addContracts = async (task: ContractTracker<any>) => {
    const latestSyncedBlockIndexerBlock =
      await this.store.indexer.getLastIndexedBlockNumberByName("blocks");
    ...
    const startBlock =
      latestSyncedBlock && latestSyncedBlock > 0 ? latestSyncedBlock + 1 : 0;
  ...
  trackEvents = async (task: ContractTracker<any>) => {
    ...
    const latestSyncedBlockIndexerBlock =
      await this.store.indexer.getLastIndexedBlockNumberByName("logs");

    const tokensIterator = EntityIterator(
      (task.tableName as EntityIteratorEntities) ||
        (task.name as EntityIteratorEntities),
      {
        batchSize: 1,
        index: 0,
      }
    ...
    for await (const tokens of tokensIterator) {
      ...
      const latestSyncedBlock = await task.trackEvents.getLastSyncedBlock(
        this.store,
        token
      );
      const startBlock =
      ...
      l.info(
        `Getting logs for "${token.name}" ${token.address} from block ${startBlock}`
      );
      ...
      for await (const logs of logsIterator) {
        ...
        try {
          ...
          latestSyncedTokenBlock = Math.max(
            ...
            logs.reduce(
              (acc, o) => (acc > o.blockNumber ? acc : o.blockNumber),
              0
            )
          );
