...
export const trackEvents = async (
  ...
  const transferLogs = logs.filter(({ topics }) =>
  ...
  if (transferLogs.length > 0) {
    ...
    const updateBalancesPromises = [...addressesToUpdate.values()].map(
      (address) => store.erc20.setNeedUpdateBalance(address, tokenAddress)
    );
    const addEventsPromises = transferEvents.map((e) =>
  ...
  const approveLogs = logs.filter(({ topics }) =>
    topics.includes(approveSignature)
  );
  if (approveLogs.length > 0) {
    const approveEvents = approveLogs.map((log) => {
      ...
      const {
        ...
        _spender: spender,
        _value: value,
      } = decodeLog(ContractEventType.Approval, log.data, topics);
      return {
    ...
    await Promise.all(
      approveEvents.map((e) => store.contract.addContractEvent(e))
    );
  }
};
