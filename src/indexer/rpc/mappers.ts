  ...
  */
  ...
  return {
    ...
    stakingTransactions:
      ...
      block.stakingTransactions.map(mapStakingTransaction),
  } as Block;
};
...
export const mapInternalTransactionFromBlockTrace =
  (blockNumber: BlockNumber) =>
  (tx: RPCInternalTransactionFromBlockTrace, i: number) => {
    ...
    return {
      ...
      to:
        tx.action.to ||
        tx.action.address ||
        (tx.result ? tx.result.address : null),
      ...
      */
      ...
      deployedBytecode:
        tx.result && tx.result.code ? tx.result.code : undefined,
