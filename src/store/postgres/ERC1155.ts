...
export class PostgresStorageERC1155 implements IStorageERC1155 {
  ...
  getERC1155LastSyncedBlock = async (address: Address): Promise<number> => {
    const res = await this.query(
      `select last_update_block_number from erc1155 where address=$1;`,
      [address]
    );
  ...
  setERC1155LastSyncedBlock = async (
    address: Address,
    blockNumber: BlockNumber
  ) => {
    return this.query(
      `update erc1155 set last_update_block_number=$1 where address=$2;`,
      [blockNumber, address]
    );
  };

  getERC721Assets = async (
    owner: Address,
    token: Address
  ...
  getTokenBalances = async (address: Address): Promise<IERC721Asset[]> => {
    const res = await this.query(
      `select * from erc1155_balance where token_address=$1`,
      [address]
    );
  ...
  getTokenAssets = async (address: Address): Promise<IERC721Asset[]> => {
    const res = await this.query(
      `select * from erc1155_asset where token_address=$1`,
      [address]
    );
