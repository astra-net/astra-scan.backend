...
export class PostgresStorageERC20 implements IStorageERC20 {
  ...
  addERC20 = async (erc20: IERC20) => {
    ...
    return await this.query(
      `insert into erc20 ${query} on conflict (address) do nothing;`,
      params
    );
  };
  ...
  getERC20LastSyncedBlock = async (address: Address): Promise<number> => {
    const res = await this.query(
      `select last_update_block_number from erc20 where address=$1;`,
      [address]
    );
  ...
  setERC20LastSyncedBlock = async (
    address: Address,
    blockNumber: BlockNumber
  ) => {
    return this.query(
      `update erc20 set last_update_block_number=$1 where address=$2;`,
      [blockNumber, address]
    );
  };
  ...
  getERC20CirculatingSupply = async (
    token: Address
  ): Promise<string | null> => {
    const res = await this.query(
      `select sum(balance) from erc20_balance where token_address=$1`,
      [token]
    );
