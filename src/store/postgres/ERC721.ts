...
export class PostgresStorageERC721 implements IStorageERC721 {
  ...
  addERC721 = async (erc721: IERC721) => {
    ...
    return await this.query(
      `insert into erc721 ${query} on conflict (address) do nothing;`,
      params
    );
  };
  ...
  getERC721LastSyncedBlock = async (address: Address): Promise<number> => {
    const res = await this.query(
      `select last_update_block_number from erc721 where address=$1;`,
      [address]
    );
  ...
  setERC721LastSyncedBlock = async (
    address: Address,
    blockNumber: BlockNumber
  ) => {
    return this.query(
      `update erc721 set last_update_block_number=$1 where address=$2;`,
      [blockNumber, address]
    );
  };
  ...
  getUserAssets = async (address: Address): Promise<IERC721Asset[]> => {
    const res = await this.query(
      `select * from erc721_asset where owner_address=$1`,
      [address]
    );
